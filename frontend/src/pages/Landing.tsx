...
import { Video, MessageCircle, ArrowRight, Bot, Target, Rocket, BarChart3, Lightbulb, TrendingUp, Award, Brain } from 'lucide-react';
import { useEffect, useState } from 'react';
import type { UserResponse } from '../utils/onboardingData';

const BigItem:React.FC<{btnText:string, description:string, icon: React.ReactNode, image: string}> = ({btnText, description, icon, image}) => {
    return (
        <motion.div 
            ...
            <div className='relative flex flex-col h-full p-6 bg-white/70 backdrop-blur-2xl border-2 border-gray-200/50 rounded-3xl shadow-xl overflow-hidden'>
                ...
                <div className="relative z-10 flex flex-col h-full">
                    <div className="flex-1 flex flex-col items-center justify-center">
                        ...
                        <p className='text-sm font-medium text-gray-700 text-center mb-4 leading-relaxed' style={{ fontFamily: 'Montserrat, sans-serif' }}>
                          {description}
                        </p>
                    </div>
  ...
  useEffect(() => {
    ...
    if (savedResponses) {
      ...
      if (skills.length > 0) {
        ...
        setAverageScore(Math.round(skills.reduce((acc, s) => acc + s.score, 0) / skills.length));
      }
    }
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 relative overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                ...
                >
                  ...
                  <div className="relative bg-white/60 backdrop-blur-xl border-2 border-gray-200/50 rounded-2xl p-5 hover:border-[#86BC25]/50 transition-all duration-300 shadow-lg">
                    ...
                    <div className="text-xs text-gray-500 font-semibold uppercase tracking-wider" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      Verze
                    </div>
                  </div>
                </motion.div>
                ...
                <div className="relative bg-gradient-to-r from-[#86BC25]/10 to-transparent backdrop-blur-sm border-2 border-[#86BC25]/20 rounded-2xl p-5 hover:border-[#86BC25]/40 transition-all duration-300">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-bold text-black leading-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                        Připoj se k dalším studentům
                      </p>
                      <p className="text-xs text-gray-600 font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                >
                  <div className="relative bg-white/50 backdrop-blur-sm border border-gray-200/50 rounded-xl p-3 hover:border-[#86BC25]/30 transition-all duration-300">
                    ...
                    <p className="text-xs font-bold text-black text-center" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      {completedQuestions > 0 ? `${Math.min(completedQuestions, 30)} dní` : 'Start'}
                    </p>
                    <p className="text-xs text-gray-500 font-medium text-center" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      Aktivita
                    </p>
                  </div>
                </motion.div>
                  ...
                  <div className="relative bg-white/50 backdrop-blur-sm border border-gray-200/50 rounded-xl p-3 hover:border-[#86BC25]/30 transition-all duration-300">
                    <div className="flex items-center justify-center mb-1">
                      <div className="flex items-center gap-0.5">
                        ...
                        <div className="w-2 h-2 rounded-full bg-[#86BC25]/60 animate-pulse" style={{ animationDelay: '0.2s' }}></div>
                        <div className="w-2 h-2 rounded-full bg-[#86BC25]/40 animate-pulse" style={{ animationDelay: '0.4s' }}></div>
                      </div>
                    </div>
                    <p className="text-xs font-bold text-black text-center" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                ...
                >
                  <div className="relative bg-white/50 backdrop-blur-sm border border-gray-200/50 rounded-xl p-3 hover:border-[#86BC25]/30 transition-all duration-300">
                    ...
                    <p className="text-xs font-bold text-black text-center truncate" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      {topSkill || '--'}
                    </p>
                    <p className="text-xs text-gray-500 font-medium text-center" style={{ fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <section className="py-32 px-6 relative">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <h2 className="text-4xl lg:text-5xl font-bold text-black mb-6 tracking-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              Proč Školník?
            </h2>
            <p className="text-lg text-gray-500 max-w-2xl mx-auto font-light" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              Jsme nový startup, který věří, že každý student zaslouží najít svou cestu
            </p>
          </motion.div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Target, title: 'Najdi své síly', desc: 'Objevuj své skryté talenty a zájmy, které tě baví' },
              { icon: Rocket, title: 'Rychlý start', desc: 'Za pár minut získáš jasný plán, kam dál' },
              { icon: BarChart3, title: '100% zdarma', desc: 'Věříme, že vzdělání musí být dostupné pro všechny' },
              { icon: Lightbulb, title: 'Tvoje tempo', desc: 'Nikdo tě netlaká, postupuj svým tempem' }
            ].map((feature, index) => {
                  ...
                  <div className="relative bg-white/60 backdrop-blur-xl border-2 border-gray-200/50 rounded-2xl p-8 hover:border-[#86BC25]/50 transition-all duration-300 shadow-lg hover:shadow-xl overflow-hidden">
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="inline-flex p-4 rounded-xl bg-black/5 backdrop-blur-sm border border-gray-200/30 mb-6 group-hover:bg-[#86BC25]/10 group-hover:border-[#86BC25]/30 transition-all duration-300">
                        <IconComponent className="w-8 h-8 text-black group-hover:text-[#86BC25] transition-colors" strokeWidth={2.5} />
                      </div>
                      
                      <h3 className="text-xl font-bold text-black mb-3 tracking-tight group-hover:text-[#86BC25] transition-colors" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                        {feature.title}
                      </h3>
                      <p className="text-gray-600 font-normal leading-relaxed" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                        {feature.desc}
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            <div className="relative bg-white/70 backdrop-blur-2xl border-2 border-gray-200/50 rounded-3xl p-16 lg:p-20 hover:border-[#86BC25]/50 transition-all duration-500 shadow-2xl overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xl text-gray-600 mb-12 max-w-2xl mx-auto font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                  Vyzkoušej Školník zdarma a objev, kam tě může tvá cesta zavést.
                </p>
      ...
      <footer className="py-16 px-6 relative border-t border-gray-200/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          >
            ...
            />
            <span className="text-lg font-black text-black tracking-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              Školník
            </span>
          </motion.div>
          <div className="text-gray-500 text-sm font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
