...
import { Brain, Sparkles, Target, Users, Zap, TrendingUp, Award, Lightbulb, Palette, Calculator, Heart, MessageCircle, Gift, Star } from 'lucide-react';
...
const CareerRevealBox = ({ responses, userName }: { responses: UserResponse[], userName: string }) => {
  ...
  const getRecommendedCareer = () => {
    ...
    const keywords = {
      tech: ['programování', 'počítač', 'technolog', 'it', 'kód', 'software', 'web', 'aplikac'],
      creative: ['kreslení', 'umění', 'design', 'kreativ', 'hudba', 'fotograf', 'grafik'],
      science: ['věda', 'výzkum', 'experiment', 'matemat', 'fyzik', 'chemie', 'biolog'],
      business: ['podnikání', 'obchod', 'marketing', 'prodej', 'ekonom', 'finance', 'management'],
      medical: ['medicín', 'lékař', 'zdraví', 'pacient', 'nemocnic', 'léčb'],
      social: ['pomoc', 'lidé', 'společnost', 'sociální', 'psycholog', 'učitel', 'vzdělávání'],
      engineering: ['stavb', 'inženýr', 'konstrukce', 'projekt', 'strojírenství', 'elektro'],
    ...
    const careers = {
      tech: { title: 'Software Developer / IT Specialista', icon: '💻', desc: 'Tvůj analytický um a zájem o technologie tě předurčují pro kariéru v IT', bg: 'from-gray-100 to-gray-50' },
      creative: { title: 'Grafický Designer / Kreativní Director', icon: '🎨', desc: 'Tvá kreativita a umělecký talent jsou tvou největší silou', bg: 'from-gray-100 to-gray-50' },
      science: { title: 'Vědecký Výzkumník / Inženýr', icon: '🔬', desc: 'Tvá zvídavost a analytické myšlení tě vedou k vědě', bg: 'from-gray-100 to-gray-50' },
      business: { title: 'Business Analyst / Entrepreneur', icon: '📊', desc: 'Máš talent pro obchod a strategické myšlení', bg: 'from-gray-100 to-gray-50' },
      medical: { title: 'Lékař / Health Care Professional', icon: '⚕️', desc: 'Tvá empatie a zájem o zdraví jsou klíčové', bg: 'from-gray-100 to-gray-50' },
      social: { title: 'Psycholog / Sociální Pracovník', icon: '🤝', desc: 'Tvůj zájem o lidi a společnost je tvým posláním', bg: 'from-gray-100 to-gray-50' },
      engineering: { title: 'Inženýr / Architekt', icon: '🏗️', desc: 'Tvé technické myšlení a zájem o projekty tě posouvají vpřed', bg: 'from-gray-100 to-gray-50' },
      arts: { title: 'Umělec / Content Creator', icon: '🎭', desc: 'Tvá kreativita a umělecká vize jsou nezaměnitelné', bg: 'from-gray-100 to-gray-50' },
    };
        ...
        {!isRevealed ? (
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 font-medium text-lg" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                Na základě analýzy {responses.length} odpovědí jsme připravili kariérní doporučení speciálně pro tebe, {userName}
              </p>
            </div>
              ...
              <div className="relative bg-gradient-to-br from-white via-gray-50 to-white border-2 border-gray-300 rounded-3xl overflow-hidden shadow-2xl">
                ...
                >
                  <div className="text-center">
                    ...
                    <h3 className="text-3xl font-bold text-gray-400 mb-4" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      Tvá ideální kariéra
                    </h3>
                    ...
                    <div className="flex items-center justify-center gap-3 text-gray-500 group-hover:text-gray-700 transition-colors">
                      ...
                      <p className="text-sm font-semibold uppercase tracking-wider" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                        Klikni pro odhalení
                      </p>
                      <div className="w-12 h-0.5 bg-gray-300 group-hover:bg-[#86BC25] transition-colors"></div>
                    </div>
          ...
          >
            ...
            <div className="relative bg-white border-2 border-gray-200 rounded-3xl shadow-2xl overflow-hidden">
              <div className={`absolute inset-0 bg-gradient-to-br ${career.bg} opacity-50`}></div>
              ...
              <div className="relative px-12 py-16">
                <div className="text-center max-w-3xl mx-auto">
                  ...
                  >
                    <div className="inline-block px-6 py-2 bg-gray-900 text-white text-sm font-bold rounded-full mb-6" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      AI DOPORUČENÍ
                    </div>
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => navigate('/roadmap', { state: { recommendedCareer: career.title } })}
...
export const Overview = () => {
  ...
  const calculateCreativity = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      ...
      if (answer.includes('kreslení') || answer.includes('umění') || answer.includes('hudba') || 
          answer.includes('psaní') || answer.includes('kreativ')) {
        score += 10;
      }
    });
  ...
  const calculateLogic = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      ...
      if (answer.includes('matemat') || answer.includes('fyzik') || answer.includes('programování') || 
          answer.includes('věda') || answer.includes('logick')) {
        score += 10;
      }
    });
    return Math.min(score, 100);
  ...
  const calculateCommunication = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      const answer = response.answer.toLowerCase();
      if (answer.includes('tým') || answer.includes('skupin') || answer.includes('diskuz') || 
          answer.includes('komunikac') || answer.includes('ostatní')) {
        score += 10;
      }
    });
    return Math.min(score, 100);
  ...
  const calculateAnalytical = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      const answer = response.answer.toLowerCase();
      if (answer.includes('analyz') || answer.includes('data') || answer.includes('čísl') || 
          answer.includes('statistik') || answer.includes('výzkum')) {
        score += 10;
      }
    });
    return Math.min(score, 100);
  ...
  const calculateTeamwork = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      const answer = response.answer.toLowerCase();
      if (answer.includes('tým') || answer.includes('skupin') || answer.includes('spolupráce') || 
  ...
  const calculateMotivation = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      const answer = response.answer.toLowerCase();
      if (answer.includes('motivac') || answer.includes('cíl') || answer.includes('úspěch') || 
          answer.includes('vynikající') || answer.includes('nejlepší')) {
        score += 8;
      }
    });
    return Math.min(score, 100);
  ...
  const calculateEmpathy = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      const answer = response.answer.toLowerCase();
      if (answer.includes('pomoc') || answer.includes('lidé') || answer.includes('péče') || 
          answer.includes('porozumění') || answer.includes('empatie')) {
        score += 10;
      }
    });
    return Math.min(score, 100);
  ...
  const calculateLeadership = (responses: UserResponse[]): number => {
    ...
    responses.forEach(response => {
      const answer = response.answer.toLowerCase();
      if (answer.includes('veden') || answer.includes('organizac') || answer.includes('řízení') || 
          answer.includes('vedoucí') || answer.includes('iniciativa')) {
        score += 10;
      }
    });
    return Math.min(score, 100);
  ...
  const averageScore = skills.length > 0 ? Math.round(skills.reduce((acc, skill) => acc + skill.score, 0) / skills.length) : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 pt-24 pb-12 px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-5xl font-bold text-gray-900 mb-3" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            Ahoj, {userName}! 👋
          </h1>
        ...
        >
          <h2 className="text-3xl font-bold text-gray-900 mb-6 flex items-center gap-3" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            ...
            {skills.map((skill, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-lg font-bold text-gray-700" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                    {skill.score}%
                  </span>
                </div>
        ...
        >
          <h2 className="text-3xl font-bold text-white mb-6 flex items-center gap-3" style={{ fontFamily: 'Montserrat, sans-serif' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {topSkills.map((skill, index) => (
              ...
              >
                ...
                <div className="text-6xl font-bold text-white mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                  #{index + 1}
                </div>
                <h3 className="text-xl font-bold text-white mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                  {skill.name}
                </h3>
                <div className="text-3xl font-bold text-white/90" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                  {skill.score}%
                </div>
              </motion.div>
            ))}
        ...
        >
          ...
          <div className="space-y-4">
            {topSkills[0] && (
              <div className="p-6 bg-gradient-to-r from-gray-100 to-transparent rounded-2xl border-l-4 border-gray-900 hover:border-[#86BC25] transition-all duration-300">
                ...
                <p className="text-gray-700 font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                  S tvým talentem v této oblasti bys mohl/a excelovat v oborech jako design, umění, marketing nebo inovace.
                </p>
              </div>
            )}
            ...
            <div className="p-6 bg-gradient-to-r from-gray-100 to-transparent rounded-2xl border-l-4 border-gray-700 hover:border-[#86BC25] transition-all duration-300">
              ...
              <p className="text-gray-700 font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                Tvoje top 3 dovednosti ti dávají skvělý základ. Zaměř se na projekty, které je kombinují!
              </p>
            </div>
            ...
            <div className="p-6 bg-gradient-to-r from-gray-100 to-transparent rounded-2xl border-l-4 border-gray-500 hover:border-[#86BC25] transition-all duration-300">
              ...
              <p className="text-gray-700 font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                Na základě tvého profilu ti doporučujeme prozkoumать střední školy se zaměřením na tvoje silné stránky.
              </p>
            </div>
          </div>
        </motion.div>
