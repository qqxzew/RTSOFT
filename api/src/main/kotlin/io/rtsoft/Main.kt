...
fun main() {
    ...
    val server = simpleServer {
        ...
        route("/__prompt__") {
            GET { req ->
                val authHeader = req.headers["Authorization"] ?: return@GET buildResponse {
                    headers["Content-Type"] = "application/json"
                    status = 401; body = """{"error":"no token"}"""
                }
                val token = authHeader.removePrefix("Bearer ").trim()
                val username = verifyToken(token) ?: return@GET buildResponse {
                    headers["Content-Type"] = "application/json"
                    status = 403; body = """{"error":"invalid token"}"""
                }
                ...
                return@GET when {
                    aiResponse != null -> aiResponse
                        ...
                        headers["Content-Type"] = "application/json"
                        status = 500; body = """{"error":"AI backend error"}"""
                    }
                }
            }
        }
